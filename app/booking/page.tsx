...
export default function BookingPage() {
  ...
  // FUNGSI INTI: Cek Kamar Berdasarkan Jam Input Tamu
  const cekKetersediaanRealTime = async (tgl: string, jam: string) => {
    if (!tgl || !jam) return;

    // Ambil kamar yang (Statusnya Tersedia) 
    // ATAU (Terisi tapi jam_kosongnya <= jam kedatangan tamu)
    ...
    if (!error && data) {
      setKamarTersedia(data);
    }
  };

  // Jalankan cek setiap kali tanggal atau jam diubah
  useEffect(() => {
  ...
  async function kirimBooking(e: any) {
    ...
    if (!error) {
      alert("✅ Berhasil! Reservasi sudah tercatat.");
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6 font-sans">
      <div className="bg-white p-8 rounded-[32px] shadow-2xl w-full max-w-md border border-slate-200">
        ...
        <p className="text-sm text-slate-500 mb-8 font-medium">Booking otomatis sesuai ketersediaan jam.</p>
        ...
        <form onSubmit={kirimBooking} className="flex flex-col gap-5">
          <div>
            ...
            <input required type="text" value={form.nama} onChange={(e) => setForm({ ...form, nama: e.target.value })} className="w-full mt-1.5 p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-semibold text-slate-900" placeholder="Siapa nama lu?" />
          </div>

          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">WhatsApp</label>
            <input required type="tel" value={form.whatsapp} onChange={(e) => setForm({ ...form, whatsapp: e.target.value })} className="w-full mt-1.5 p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-semibold text-slate-900" placeholder="08xxx" />
          </div>
          ...
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Pilih Kamar (Tersedia di jam tersebut)</label>
            <select 
              required 
              disabled={!form.jam_kedatangan}
              value={form.tipe_kamar} 
              onChange={(e) => setForm({ ...form, tipe_kamar: e.target.value })} 
              className="w-full mt-1.5 p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-semibold text-slate-900 appearance-none disabled:opacity-50"
            >
              {!form.jam_kedatangan ? (
                <option>Silahkan pilih jam dulu...</option>
              ) : kamarTersedia.length === 0 ? (
                <option>❌ Maaf, jam tersebut penuh!</option>
              ) : (
                <>
                  <option value="">-- Pilih Kamar --</option>
                  {kamarTersedia.map((item) => (
                    <option key={item.id} value={`Kamar ${item.no_kamar} (${item.tipe})`}>
                      Unit {item.no_kamar} - {item.tipe}
                    </option>
                  ))}
                </>
              )}
          ...
          <button 
            type="submit" 
            disabled={kamarTersedia.length === 0}
            className="w-full mt-2 py-4 bg-slate-900 text-white text-sm font-bold rounded-2xl hover:bg-slate-800 transition-all active:scale-95 disabled:bg-slate-300 shadow-lg shadow-slate-200"
          >
            KONFIRMASI BOOKING 🚀
          </button>
        </form>
